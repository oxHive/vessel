...
async fn full_generate_save_retrieve_flow() {
    ...
    let profile = db::profiles::create(&db, "TestProfile", voice)
        .await
        .unwrap();
    ...
    let generation = db::generations::create(&db, &project.id, "v1.0.0", "release", None)
        .await
        .unwrap();
    db::generations::save_output(
        &db,
        &generation.id,
        "twitter",
        "My project v1.0.0 is out! #opensource",
    )
    .await
    .unwrap();
    db::generations::save_output(
        ...
        &generation.id,
        "discord",
        "**v1.0.0** is live — check the README for details.",
    )
    .await
    .unwrap();

    // verify via DB
    let (fetched_gen, outputs) = db::generations::get_with_outputs(&db, &generation.id)
        .await
        .unwrap()
        .unwrap();
